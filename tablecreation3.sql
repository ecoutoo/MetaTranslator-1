...
CREATE TABLE SurveyResults (id SERIAL, sentence INTEGER, group_id INTEGER, correct BOOLEAN, problem_phrase VARCHAR(1000), confidence INTEGER, own_translation VARCHAR(1000), FOREIGN KEY (sentence) REFERENCES Sentences(id) ON DELETE RESTRICT ON UPDATE CASCADE, FOREIGN KEY (group_id) REFERENCES Surveys(id) ON DELETE RESTRICT ON UPDATE CASCADE);

DROP TABLE IF EXISTS UserSurveyRecord cascade;
CREATE TABLE UserSurveyRecord (id SERIAL, user_id INTEGER, survey_id INTEGER, PRIMARY KEY (id), FOREIGN KEY (user_id) REFERENCES Users(id) ON DELETE RESTRICT ON UPDATE CASCADE, FOREIGN KEY (survey_id) REFERENCES Surveys(id) ON DELETE RESTRICT ON UPDATE CASCADE);
